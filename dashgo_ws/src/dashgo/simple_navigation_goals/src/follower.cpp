...
#define GOHOME "HOME"
std::vector<double> m_VpersonX;
std::vector<double> m_VpersonY;
int    m_PersonMeanSize=4;
int    m_PersonNum=0;
double person_x=0;
double person_y=0;
double person_yaw=0;
...
int main(int argc, char **argv)
{
  ...
  while (n.ok())
  {
	...
	m_VpersonX.push_back(transform.getOrigin().x());
	m_VpersonY.push_back(transform.getOrigin().y());
	m_PersonNum++;
        //ROS_INFO("it is person_x=%d",m_PersonNum);
	if (m_PersonNum>=m_PersonMeanSize)
	{
		std::vector<double>::iterator itrx=m_VpersonX.begin();
		std::vector<double>::iterator itry=m_VpersonY.begin();
		double x_sumTem=0;
		double y_sumTem=0;
		for (;itrx!=m_VpersonX.begin()+m_PersonMeanSize;itrx++,itry++)
		{
			x_sumTem+=*itrx;
			y_sumTem+=*itry;
		}
		person_x=x_sumTem/double(m_PersonMeanSize);
		person_y=y_sumTem/double(m_PersonMeanSize);
		...
		{  
			...
			goal_pose.or_w=1.0;
			setGoal(goal_pose);
			ROS_INFO("send goal,goal_succeed = %d ", goal_succeed);
		}
	}
	/*else
	{
